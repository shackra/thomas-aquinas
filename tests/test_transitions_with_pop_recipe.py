...
# As to the present summa revision (r1069), the only concrete transition scene
...
While you can apply any summa transition scene to the scene changes
director.run
director.replace
director.push
...
But is easy to implement in your app with the following recipe:
    ...
    1. select one of the stock scene transitions, to be found in
       summa/scenes/transitions.py, say ZzzTransition
...
description = """
    ...
    same as before the first transition initiated; that's 0 for summa rev>1066
    """
