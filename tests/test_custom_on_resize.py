...
This script demonstrates:
    How to listen to director events, in particular the on_summa_resize event.
...
class AutocenteredBackgroundLayer(summa.layer.Layer):
    ...
    For doing that, it registers to the event on_summa_resize and repositions
    ...
    def on_enter(self):
        ...
        director.push_handlers(self.on_summa_resize)
        if autotest:
    ...
    def on_exit(self):
        ...
        director.remove_handlers(self.on_summa_resize)
        ...
        # so we do here a direct call to on_summa_resize
        self.on_summa_resize(director._usable_width, director._usable_height)
    ...
    def on_summa_resize(self, usable_width, usable_height):
