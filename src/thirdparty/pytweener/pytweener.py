...
or function with a corresponding
            change value. The tween will throw an error if you specify 
an attribute the object does
            not possess. Also the data types of the change and the 
initial value of the tweened item
            must match. If you specify a 'set' -type function, the 
tweener will attempt to get the
            starting value by call the corresponding 'get' function on 
the object. If you specify a 
            property, the tweener will read the current state as the 
starting value. You add both 
            functions and property changes to the same tween.
 
            in addition to any properties you specify on the object, 
these keywords do additional
            setup of the tween.
 
            tweenTime = the duration of the motion
            tweenType = one of the predefined tweening equations or your 
own function
            onCompleteFunction = specify a function to call on 
completion of the tween
            onUpdateFunction = specify a function to call every time the 
tween updates
        ...
        tw = Tween( obj, t_time, t_type, t_completeFunc, t_updateFunc, 
t_delay, **kwargs )
        if tw:    
...
Tweener.addTween( ... )
    ...
    def decodeArguments(self):
        ...
        if len(self.tweenables) == 0:
            # nothing to do 
            print "TWEEN ERROR: No Tweenable properties or functions 
defined"
            self.complete = True
            return
        ...
        # check that its compatible
            if not hasattr( self.target, k):
                print "TWEEN ERROR: " + str(self.target) + " has no 
function " + k
                self.complete = True
                break
...
tweenable.startValue, tweenable.change, self.duration )
            for funcName, func, tweenable in self.tFuncs:
                func( self.tween( self.delta, tweenable.startValue, 
...
original
        ...
        Allows the parameters of tweens to be changed at runtime. The 
parameters
        can even be tweened themselves!
 
        eg:
 
        # the rocket needs to escape!! - we're already moving, but must 
go faster!
        twn = tweener.getTweensAffectingObject( myRocket )[0]
        tweenable = twn.getTweenable( "thrusterPower" )
        tweener.addTween( tweenable, change=1000.0, tweenTime=0.4, 
tweenType=tweener.IN_QUAD )
...
if __name__=="__main__":
    ...
    T = Tweener()
    tst = TweenTestObject()
    mt = T.addTween( tst, setRotation=500.0, tweenTime=2.5, 
					tweenType=T.OUT_EXPO, 
                    	  pos=-200, tweenDelay=0.4, 
